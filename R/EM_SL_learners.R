# learner functions that might help to pick up effect modification, and therefore
# find a more optimal tx
#' @export
...
#' glmnet wrapper that handles probabilities instead of just binary 1's and 0's
#' @export
SL.glmnetprob <- function(Y, X, newX, family, obsWeights, id, alpha = 1, nfolds = 10, 
...
#' @export
SL.glmnetem <- function(Y, X, newX, family, obsWeights, id, alpha = 1, nfolds = 10, 
    nlambda = 100, useMin = TRUE, ...) {
    require("glmnet")
    if (!is.matrix(X)) {
